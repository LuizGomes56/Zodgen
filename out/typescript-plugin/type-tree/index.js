...
exports.getTypeInfoAtPosition = getTypeInfoAtPosition;
const get_ast_node_1 = require("./get-ast-node");
let typescript;
let checker;
let options = {
 ...
 */
function getTypeInfoAtPosition(typescriptContext, typeChecker, sourceFile, position, prettifyOptions) {
    try {
        typescript = typescriptContext;
        checker = typeChecker;
        options = prettifyOptions;
        const node = (0, get_ast_node_1.getDescendantAtRange)(typescript, sourceFile, [position, position]);
        ...
        if (syntaxKind === typescript.SyntaxKind.ClassDeclaration && // Confirm the node is a class
...
function isPartOfImportStatement(node) {
    while (node) {
...
function getVariableDeclarationKind(node) {
    ...
    if (!typescript.isVariableDeclarationList(parent))
...
function getConstructorTypeInfo(type, typeChecker, name) {
 ...
 */
function getTypeTree(type, depth, visited) {
    ...
    if (typeName.startsWith('Promise<')) {
        if (!options.unwrapPromises && !typeName.includes('{'))
            return {
                kind: 'reference',
                typeName
            };
        const typeArgument = checker.getTypeArguments(apparentType)[0];
        const promiseType = typeArgument
    ...
    if (checker.isArrayType(apparentType)) {
        ...
        const arrayType = checker.getTypeArguments(apparentType)[0];
        const elementType = arrayType
    ...
    if (apparentType.isClassOrInterface() ||
        ...
        const properties = typeProperties.map(symbol => {
...
function isPrimitiveType(type) {
    ...
    if (typeFlags & typescript.TypeFlags.EnumLike)
        return false;
    return Boolean(typeFlags & typescript.TypeFlags.String ||
 ...
 */
function isPublicProperty(symbol) {
    ...
    return declarations.every(declaration => {
        if (!(typescript.isMethodDeclaration(declaration) ||
...
function isReadOnly(symbol) {
    ...
    if (!declarations)
        return false;
    return declarations.some(declaration => ((typescript.isPropertyDeclaration(declaration) ||
...
function isOptional(symbol) {
    ...
    if (!declarations)
        return false;
    return declarations.some(declaration => (typescript.isPropertySignature(declaration) ||
        typescript.isPropertyDeclaration(declaration)) && !!declaration.questionToken);
}
/**
 * Check if a declaration has members
 */
function hasMembers(declaration) {
 ...
 */
function getIndexIdentifierName(type, signature) {
